...
`utils/hamlet.py` ports HAMLET (arXiv 2510.00695): `n_moment` (8) learnable **moment tokens** are spliced into every turn right after `<|vision_end|>` as placeholder ids above the tokenizer vocabulary (spare rows of the embedding matrix; tokenizer untouched), and a 2-layer block-causal **memory transformer** over the moment hidden states of the *previous* turns produces a read-out that enters the LM as a **memory token** (`n_mem`, default 1) spliced right before the turn's `<|vision_start|>`: `MEM_t = mem_embed + out_proj(memory(m'_{<t}))`, so the frame, the moment tokens and the decision token all attend to memory. The training replay is one causal forward over stored embeds and cannot feed its own outputs back into its input, so the rollout stores every turn's moment hidden states (`moment_history` in the packed episode) and `forward_embeds_core` rebuilds the memory rows from those with the current module parameters — gradient reaches the whole module and (through the input row) the LoRA, but not the moment hidden states themselves; `hamlet_moment_drift` logs the staleness. Rollout (`_hamlet_prepare_turn` before the forward, `_hamlet_record_moments` after) and replay run the same per-block `readout`. The module lives on the base model as `model.hamlet` (attached in `load_model`, before PEFT) and rides in the adapter via `modules_to_save`; `disable_adapter()` therefore routes the ref-policy pass through its untrained copy, which is why `out_proj` must stay zero-init (memory token == constant `mem_embed`). Requires `use_sparse=True`; refuses `attn_weighting='grad'`. `tests/hamlet_unit.py` (CPU) pins the read-out contract (zero at init, strict causality, rollout form == replay form, window, gradient to every parameter, splice layout); `tests/hamlet_smoke.py` (GPU) pins the pipeline invariants (one memory + one moment block per decision, ratio==0 at init, replay==rollout, ref==old at init, gradients reach the module, checkpoint round-trip, eviction keeps the moment history). The memory history lives outside the KV cache, so it survives `context_window` eviction — that combination is the intended eval ablation. Training is LoRA/PEFT in `VLMTrainingMixin` (`utils/vlm_worker.py`; `utils/trainers.py` is the dormant TRL SFT trainer), with losses and advantage estimators pulled from the vendored `verl` submodule. Stage-1 RL used reward shaping `collision_penalty=0.02, explr_bonus=0.03`, no fp-stop penalty; `cluster/delta/run_rl_train.sh` pins those (schema defaults differ) and starts the LoRA lr at 1.25e-6, where stage 1 ended.
